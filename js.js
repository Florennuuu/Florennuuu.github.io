

document.getElementById('boton').addEventListener('click',function(){
console.log('holamundo')
});

document.getElementById('boton_color').addEventListener('click',function(){
    console.log('botoncolor')
    });
    
document.getElementById('ocultar_info').addEventListener('click',function(){
    document.getElementById('info').style.display = 'none';
    });


    function onHover()
    {
        $("#menuImg").attr('src', 'https://image.similarpng.com/very-thumbnail/2020/05/Vector-Behance-icon-PNG.png');
    
    }
    
    function offHover()
    {
        $("#menuImg").attr('src', 'https://icons.veryicon.com/png/o/miscellaneous/eva-icon-fill/behance-63.png');
       
    }
    


    function onHover2()
    {
       
        $("#menuGit").attr('src', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRRjoL1JxKlBqeV2z1YKZ4QtRmnfabIh6q0A&usqp=CAU');
    }
    
    function offHover2()
    {
       
        $("#menuGit").attr('src', 'https://cdn.freebiesupply.com/logos/thumbs/2x/github-icon-1-logo.png');
    }



    function onHover3()
    {
       
        $("#menuLinkedin").attr('src', 'https://cdn-icons-png.flaticon.com/512/174/174857.png');
    }
    
    function offHover3()
    {
       
        $("#menuLinkedin").attr('src', 'data:image/png;base64,');
    }